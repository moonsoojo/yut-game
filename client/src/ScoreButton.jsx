...
export default function ScoreButton({ position, device='portrait' }) {
  ...
  return (
    ...
    <group position={[0, 0, 0]}>
      ...
      <Text3D font="/fonts/Luckiest Guy_Regular.json" size={0.5} height={0.01} rotation={layout[device].scoreButton.rotation}>
        Score
        <meshStandardMaterial color={hoverScoreText ? "white" : "yellow"} />
      </Text3D>
      { displayScoreOptions && <group position={[0, 0, 0.5]}>
        <Text3D font="/fonts/Luckiest Guy_Regular.json" size={0.3} height={0.01} rotation={layout[device].scoreButton.rotation}>
          Finish with
          <meshStandardMaterial color={hoverScoreText ? "white" : "yellow"} />
        </Text3D>
